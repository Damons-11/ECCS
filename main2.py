...
class UISeesApp(ctk.CTk):
    ...
    def open_ocpp_debug_window(self, event=None):
        ...
        self.debug_window.geometry("540x800")  
        ...
        params = [
            ("RFID ID Tag / Token", "CARD_DEV_01", "rfid_tag"), 
    ...
    def render_station_summary_view(self):
        # Sengaja ditinggikan agar jarak tombol jelas dan rapi
        self.summary_card = ctk.CTkFrame(self.main_content_frame, width=640, height=620, fg_color="#191C22", corner_radius=24, border_color="#292D36", border_width=1)
        ...
        lbl_title_sum.place(relx=0.5, y=60, anchor=tk.CENTER)
        ...
        lbl_subtitle.place(relx=0.5, y=100, anchor=tk.CENTER)

        details_box = ctk.CTkFrame(self.summary_card, width=540, height=220, fg_color="#101319", corner_radius=16)
        details_box.place(relx=0.5, y=250, anchor=tk.CENTER)
        ...
        # --- [TOMBOL 1] TOMBOL UTAMA (Selesaikan Sesi, Harus Cabut Kabel Terlebih Dahulu) ---
        self.btn_back_summary = ctk.CTkButton(
            self.summary_card, 
            text="Selesaikan Sesi & Reset Konektor", 
            font=("Hanken Grotesk", 16, "bold"), 
            fg_color="#184C75", 
            hover_color="#1C5B8C", 
            text_color="#E1E2EB", 
            width=460, 
            height=54, 
            corner_radius=12,
            command=self.handle_return_to_main_home
        )
        self.btn_back_summary.place(relx=0.5, y=440, anchor=tk.CENTER)
        
        # --- [TOMBOL 2] TOMBOL MULTITASKING (Kembali Ke Home Tanpa Beres / Putus Sesi) ---
        # Diperbaiki agar warnanya solid, terlihat jelas, dan tidak transparan samar lagi
        self.btn_home_safe = ctk.CTkButton(
            self.summary_card, 
            text="🏠 Kembali ke Home (Tanpa Close Sesi / Multitasking)", 
            font=("Geist", 13, "bold"), 
            fg_color="#2A2F3D", 
            hover_color="#3D4559", 
            border_color="#4F576B", 
            ...
            height=46, 
            corner_radius=12,
            command=self.navigate_to_home_safe
        )
        self.btn_home_safe.place(relx=0.5, y=515, anchor=tk.CENTER)
    ...
    def update_ui_loop(self):
        try:
            ...
            # 1. Deteksi Transisi Mulai Charging
            ...
            # 2. Deteksi Transisi Berhenti Charging (Pindah ke Summary)
                ...
                self.gif_animation_active[cid] = False 
                self.start_times[cid] = None
            ...
            # 🔥 [BARIS PERBAIKAN] Simpan status sekarang agar loop detik berikutnya tahu status sebelumnya
            self.last_known_statuses[cid] = current_status

            # 3. Sinkronisasi Data Live Saat Halaman Charging Aktif
            ...
            # 4. Sinkronisasi Tombol pada Halaman Home
            ...
            # 5. Sinkronisasi Kunci Tombol pada Halaman Summary
                ...
                if hasattr(self, 'btn_back_summary') and self.btn_back_summary.winfo_exists():
                    if current_status == "FINISHING":
                        self.btn_back_summary.configure(
                            state="disabled",
                            text="🔒 Menunggu Kabel Dicabut (Status Available)...",
                            fg_color="#242831",
                            text_color="#555E70"
                        )
                    else:
